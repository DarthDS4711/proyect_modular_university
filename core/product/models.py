...
class Product(models.Model):
    ...
    pvp = models.DecimalField(
        verbose_name='pvp', max_digits=9, decimal_places=2)
    # imagen del producto
    image = models.ImageField(
        upload_to='product/%Y/%m/%d', null=True, blank=True, verbose_name='image')
    # descuento del producto
    discount = models.DecimalField(
        verbose_name='discount', max_digits=3, decimal_places=2)
...
class Comment(models.Model):
    ...
    product = models.ForeignKey(
        Product, verbose_name='product_id', on_delete=models.PROTECT)
    user = models.ForeignKey(
        User, verbose_name='user_id', on_delete=models.PROTECT)
    product_raiting = models.IntegerField(
        default=0, verbose_name='product_raiting')
